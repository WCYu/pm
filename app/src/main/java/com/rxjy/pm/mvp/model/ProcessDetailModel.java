...
public class ProcessDetailModel implements ProcessDetailContract.Model {
    @Override
    public Observable<String> getProcessDetail(int processID) {
        return ApiEngine.getInstance().getGcApiService()
                .getProcessDetail(processID, 1)
    ...
    public Observable<String> subProcessDetailPhoto(int stepID, int processID, int stepPhotoID, String url) {
        ...
        return ApiEngine.getInstance().getGcApiService()
                .subProcessPhoto(stepID, processID, stepPhotoID, 1, body)
                .compose(RxSchedulers.<String>switchThread());
    }

    @Override
    public Observable<String> subProcessDetailPhotoCamera(int stepID, int processID, int stepPhotoID, String url) {
        //将照片路径转换为File对象
        File file = new File(url);
        //创建RequestBody ,用于Retrofit2.0上传照片
        RequestBody requestFile = RequestBody.create(MediaType.parse("multipart/form-data"), file);
        ...
        return ApiEngine.getInstance().getGcApiService()
                .subProcessPhotoCamera(stepID, processID, stepPhotoID, 1, body)
                .compose(RxSchedulers.<String>switchThread());
    }
}
